...
class CreateMaterialRequestDates extends React.Component {
    constructor(props) {
        super(props);
        const {
            handleSubmit,
    ...
    handleSwitch = async () => {
        ...
        const snackMessage = Texts[language].createMaterialRequestDates.datesError;
        if (!repetition) {
    ...
    render() {
        ...
        const texts = Texts[language].createMaterialRequestDates;
        const navbar = <Navbar />;
        /*const repetitionStyle = repetition
            ? { color: "#00838F" }
            : { color: "rgba(0,0,0,0.5)" };*/
        return (
            <div id="createMaterialRequestDatesContainer">
                <h1>{texts.header}</h1>
...
CreateMaterialRequestDates.propTypes = {
...
export default withSnackbar(withLanguage(CreateMaterialRequestDates));
