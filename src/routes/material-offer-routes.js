...
router.get("/:id", (req, res, next) => {
  ...
  MaterialOffer.findOne({ material_offer_id: id })
    .then((offer) => {
      if (!offer) {
...
router.delete("/:id", async (req, res, next) => {
  ...
  let offer = await MaterialOffer.findOne({
    material_offer_id: id,
    created_by: req.user_id,
  });
  let result = await MaterialOffer.deleteOne({
    material_offer_id: id,
    created_by: req.user_id,
  });
  ...
  let futureBookings = await MaterialBooking.find({
    material_offer_id: id,
    start: { $gte: Date.now() },
  });
  await nh.materialOfferDeletedNotification(offer, futureBookings);

  MaterialBooking.deleteMany({ material_offer_id: id })
    .then(() => {
      res.json(true);
    })
    .catch(next);
...
router.post("/:id/booked", (req, res, next) => {
  ...
  MaterialOffer.findOne({
    material_offer_id: id,
    created_by: { $eq: req.user_id },
  })
    .then((offer) => {
      if (!offer) {
